...
export function orderBy<T>(retriever: (v: T) => number, desc?: boolean): (a: T, b: T) => number {
	return desc ? (b, a) => retriever(a) - retriever(b) : (a, b) => retriever(a) - retriever(b)
}
/**
 ...
 */
export function prune<T>(object: T, nanAndNull = false): Partial<T> {
	const R: Partial<T> = {}
	...
	for (const k in object) {
		if (
			object[k] === undefined ||
			(nanAndNull &&
				(object[k] === null || (typeof object[k] === 'number' && isNaN(object[k] as any))))
		) {
			continue
		}
		R[k] = object[k]
